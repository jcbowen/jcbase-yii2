...
class RegionCode
{
    ...
    {
        if (empty($this->region)) $this->region = $this->region();
    }
    ...
    public function region(): array
    {
        return [
            ...
            "country"   => [
                ...
                "10707378" => "芬兰",
                "10707473" => "斐济",
                "10707675" => "马尔维纳斯群岛（阿根、英争议）（英称福克兰群岛）",
                "10708265" => "克利珀顿岛（法）",
                "10708279" => "法罗群岛",
                "10708377" => "密克罗尼西亚联邦",
                "10716566" => "加蓬",
    ...
    public function getCityCodeByAreaCode(string $area_code = '')
    {
        ...
        foreach ($this->region['areas'] as $city_code => $area) {
            $area = array_keys($area);
            if (in_array($area_code, $area)) {
                $parent_city_code = $city_code;
                break;
            }
        }
    ...
    public function getProvinceCodeByCityCode(string $city_code = '')
    {
        ...
        foreach ($this->region['citys'] as $province_code => $city) {
            $city = array_keys($city);
            if (in_array($city_code, $city)) {
                $parent_province_code = $province_code;
                break;
            }
        }
